...
//
// JARVIS changes vs. the upstream drop-in:
//   * UNIFORMS retuned to the "Graphite & Ember" palette so the field reads as
//     slow-moving embers behind the UI rather than a rainbow.
//   * `prefers-reduced-motion` is honoured — the field renders one static frame
//     and stops animating instead of looping forever.
//   * `speed` / `paused` props so each mount can tune its own energy.
...
export interface ShaderBackgroundProps {
  className?: string
  /** Multiplier on the field's base time scale. 0 renders a single static frame. */
  speed?: number
  /** Freeze the field without unmounting the WebGL context. */
  paused?: boolean
}
...
export function ShaderBackground({
  ...
  // Read through refs so changing a prop retunes the running loop instead of
  ...
  useEffect(() => {
    ...
    // `render` below is a hoisted function declaration, so TypeScript discards
    // the null-narrowing it would otherwise carry into the closure for the
    // captured `canvas`/`gl` consts. Re-binding them with explicit non-null
    // types keeps the render loop typed without scattering `!` assertions.
    ...
    // Accumulated field time. Advancing this per frame (rather than reading the
    // wall clock) lets speed changes and reduced-motion pauses take effect
    // without the field jumping to a different point in the noise.
    let fieldTime = 0
    ...
    const effectiveSpeed = () =>
      reduceMotion || pausedRef.current
        ? 0
        : UNIFORMS.timeScale * speedRef.current
    ...
    function render(now: number) {
      ...
      lastNow = now
      const timeScale = effectiveSpeed()
      fieldTime += dt * timeScale
      ...
      ctx.uniform4f(
        ...
        width,
        height,
        // Seed the static frame off the mount time so multiple instances do not
        // render the identical field.
        fieldTime + start * 0.0001,
        UNIFORMS.colorCount,
      )
      ctx.uniform4f(
        uni.space,
      ...
      // A zero time scale (reduced motion / paused) settles into a single
      // static frame instead of burning a rAF slot every 16ms.
      if (Math.abs(timeScale) > 0.0001 || pointerSettling) requestRender()
      else lastNow = null
    }
    requestRender()
    return () => {
