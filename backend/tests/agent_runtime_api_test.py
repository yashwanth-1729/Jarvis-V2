...
async def main() -> int:
 ...
 app.state.agent_worker=ReadOnlyWorker(app.state.agent_repository,ReadOnlyFixtureAdapter({'desktop-installed-path':'installed path verified'}))
 fixture_body={**body,'client_request_id':'api-client-fixture','input':{'text':'desktop-installed-path'}}
 fixture=client.post('/api/agent/runs',headers=headers,json=fixture_body)
 await db.disconnect(); checks=[('unauthenticated request is rejected',unauth),('duplicate submit does not create another run',first.status_code==200 and second.json()['created'] is False),('event cursor replays then advances',len(replay['events'])==1 and cursor['events']==[]),('changed idempotency request conflicts',changed),('cross-principal inspection is rejected',forbidden),('installed-path fixture executes to verified completion',fixture.status_code==200 and fixture.json()['run']['status']=='COMPLETED')]
