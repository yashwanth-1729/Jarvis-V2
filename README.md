...
covers. Detail screens have back navigation; record editors are bottom sheets.
...
mute/send, stop reply and the existing half-duplex default. Motion uses eased
screen/press feedback, a sliding dock highlight and low-amplitude glass drift
with slow reflections on the voice control. Loops pause when hidden or covered
by voice mode; reduced-motion/transparency fallbacks remain. Illustration assets
will be supplied by the user, not generated by
the agent; see `docs/mobile-pocket.md` for the first asset prompt.
